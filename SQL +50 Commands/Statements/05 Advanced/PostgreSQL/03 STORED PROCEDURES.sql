...
-- En PostgreSQL no se necesita cambiar el delimitador; el bloque `BEGIN ... END` puede contener múltiples instrucciones.

-- Crear un procedimiento almacenado `p_all_users` que obtiene todos los usuarios
CREATE OR REPLACE PROCEDURE stored_procedure_all_users()
LANGUAGE plpgsql  -- Especifica el lenguaje utilizado para definir el procedimiento (PL/pgSQL en PostgreSQL)
AS $$
BEGIN
	SELECT * FROM users;
END;
...
-- Crear un procedimiento almacenado `p_age_users` con un parámetro de entrada `age_param` para filtrar por edad
CREATE OR REPLACE PROCEDURE stored_procedure_age_users(age_param INT)
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Invocar al procedimiento almacenado `p_age_users` con el valor `30` como parámetro de edad
