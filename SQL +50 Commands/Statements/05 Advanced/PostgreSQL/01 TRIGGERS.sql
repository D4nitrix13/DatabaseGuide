...
/* 
Crear el trigger `tg_email` que ejecutará la función `log_email_update` cada vez que se actualice el email en `users`.
`AFTER UPDATE ON users` asegura que el trigger se ejecute después de una actualización en la tabla.
*/
CREATE TRIGGER tg_email AFTER
...
Esto ejecutará el trigger `tg_email`, y, si el email cambió, registrará el email anterior en `email_history`.
*/
UPDATE users SET email = 'Daniel@gmail.com' WHERE user_id = 1;

/*
Eliminar el trigger `tg_email` de la tabla `users`.
